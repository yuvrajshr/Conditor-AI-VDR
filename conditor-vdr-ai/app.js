...
function srcLine(live){
  return `<div class="ai-src ${live?'live':'demo'}">${I.spark} ${live
    ? "Generated by "+CONFIG.AI_PROVIDER_LABEL+" via Conditor backend (/api/ai)"
    : "Demo intelligence (offline) — deploy with an API key for live "+CONFIG.AI_PROVIDER_LABEL}</div>`;
}
function toast(msg){
  const t=document.createElement("div"); t.textContent=msg;
  t.style.cssText="position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:var(--g800);color:#eef3ef;padding:11px 20px;border-radius:8px;font-size:13px;box-shadow:var(--shadow-lg);z-index:120;border:1px solid var(--gold);animation:rise .3s;font-weight:600";
